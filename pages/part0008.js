...
<p className="it">En este sentido, no tenemos que utilizar   <code className="calibre15">.container-fluid</code>  , ya que es fácil tener el SVG ocupa toda la pantalla en los dispositivos móviles.   En el   <code className="calibre15">splash</code>  div, que animan el uso   <code className="calibre15">steps()</code>  para el número de fotogramas que teníamos en el SVG:  </p>

<p className="p">Here, we don’t have to use <code className="calibre15">.container-fluid</code>, because it’s easy to have the SVG take up the whole screen on mobile devices. In the <code className="calibre15">splash</code> div, we animate using <code className="calibre15">steps()</code> for the number of frames we had in the SVG:</p>
...
<p className="it">El uso de un SVG en lugar de un PNG nos da la ventaja de una imagen nítida en todas las pantallas, pero es fácil de proporcionar un repliegue.   Utilizamos Modernizr para crear un gancho de clase sobre   <code className="calibre15">body</code>  y todavía puede animar con el PNG hemos creado:  </p>

<p className="p">Using an SVG rather than a PNG gives us the advantage of a crisp image on all displays, but it’s easy to provide a fallback. We use Modernizr to create a class hook on <code className="calibre15">body</code> and can still animate it with the PNG we created:</p>
