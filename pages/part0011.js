...
<li className="it">Filtración</li>
	<li className="p">Filtering</li>
<li className="it">reordenamiento</li>	
	<li className="p">Reordering</li>
<li className="it">Cuentacuentos</li>	
	<li className="p">Storytelling</li>
<li className="it">Que muestra el cambio con el tiempo</li>	
	<li className="p">Showing change over time</li>
<li className="it">piezas de escalonamiento para mayor claridad</li>	
	<li className="p">Staggering pieces for clarity</li>
</ul>
...
<p className="it">Con el fin de crear la ilusión de un dibujo SVG, que necesitamos para obtener la longitud de la ruta SVG, lo que podemos hacer con <code>.getTotalLength()</code>: <sup className="calibre42"><a data-type="noteref" id="idm140658633302864-marker" href="part0011_split_007.html#idm140658633302864" className="calibre7">2</a></sup></p>

<p className="p">In order create the illusion of an SVG drawing, we need to get the length of the SVG path, which we can do with <code>.getTotalLength()</code>:<sup className="calibre42"><a data-type="noteref" id="idm140658633302864-marker" href="part0011_split_007.html#idm140658633302864" className="calibre7">2</a></sup></p>
...
<p className="it">En primer lugar, vamos a establecer una <code>stroke-dasharray</code> de<a contenteditable="false" data-primary="stroke-dasharray" data-type="indexterm" id="sda6" className="calibre7"></a>uno de los caminos:</p>

<p className="p">First, let’s set a <code>stroke-dasharray</code> on <a contenteditable="false" data-primary="stroke-dasharray" data-type="indexterm" id="sda6" className="calibre7"></a>one of the paths:</p>
...
<p className="it">Eso puede ser tan largo como queremos que sea. También podemos establecer una <code>stroke-dashoffset</code>, que también puede ser tan largo como queremos que sea. Como el nombre<a contenteditable="false" data-primary="stroke-dashoffset" data-type="indexterm" id="sdo6" className="calibre7"></a>sugiere, <code>stroke-dashoffset</code> compensa el accidente cerebrovascular en cualquier cantidad, y, afortunadamente, también es una propiedad animatable.</p>

<p className="p">That can be as long as we want it to be. We can also set a <code>stroke-dashoffset</code>, which can also be as long as we want it to be. As the name <a contenteditable="false" data-primary="stroke-dashoffset" data-type="indexterm" id="sdo6" className="calibre7"></a>suggests, <code>stroke-dashoffset</code> offsets the stroke by any amount, and, thankfully, it’s also an animatable property.</p>

<p className="it">Ahora podemos usar la salida de la consola y nuestra pequeña tabla para crear una animación que se lleva a todo lo largo de toda la carrera y también compensa por mucho. Esto hace que nuestra <a href="http://codepen.io/sdras/full/oxNmRM" className="calibre7">visualización de datos</a> parece que fue arrastrado a las <code>viewBox</code> (ver Figuras <a href="part0011_split_004.html#fig6.10" className="calibre7">6-10</a> y <a href="part0011_split_004.html#fig6.11" className="calibre7">6-11</a> ). Estamos utilizando la misma información varias veces, por lo que podemos utilizar un mixin que se seque. También tenemos valores diferentes para el <code>dashoffset</code> y <code>dasharray</code>, por lo que la mantenga<a contenteditable="false" data-primary="DRY (Don't Repeat Yourself)" data-type="indexterm" id="idm140658633282448" className="calibre7"></a>SECO, animamos a <code>0</code> en lugar de al revés:</p>

<p className="p">We can now use the console output and our little table to create an animation that takes the full length of the whole stroke and also offsets it by that much. This makes our <a href="http://codepen.io/sdras/full/oxNmRM" className="calibre7">data visualization</a> look like it was drawn into the <code>viewBox</code> (see Figures <a href="part0011_split_004.html#fig6.10" className="calibre7">6-10</a> and <a href="part0011_split_004.html#fig6.11" className="calibre7">6-11</a>). We are using the same information a few times, so we can use a mixin to DRY it out. We also have different values for the <code>dashoffset</code> and <code>dasharray</code>, so to keep it <a contenteditable="false" data-primary="DRY (Don't Repeat Yourself)" data-type="indexterm" id="idm140658633282448" className="calibre7"></a>DRY, we animate to <code>0</code> instead of the other way around:</p>
...
<p className="it">Hemos establecido la línea de <a contenteditable="false" data-primary="d3.line()" data-type="indexterm" id="idm140658633257792" className="calibre7"></a>el <code>d3.line()</code> método, que establece los atributos de la línea a la <em className="calibre2">x</em> y <em className="calibre2">Y</em> coordenadas de dos campos de datos. Luego llamamos a una transición entre los dos estados de los atributos de línea. Opcionalmente, también podemos declarar retrasos, duraciones, y alivia.</p>

<p className="p">We set the line to <a contenteditable="false" data-primary="d3.line()" data-type="indexterm" id="idm140658633257792" className="calibre7"></a>the <code>d3.line()</code> method, which sets the attributes of the line to the <em className="calibre2">x</em> and <em className="calibre2">y</em> coordinates of two fields of data. We then call a transition between the two states of the line attributes. Optionally, we can also declare delays, durations, and eases.</p>
...
<p className="it">tambalea son <a contenteditable="false" data-primary="staggers" data-type="indexterm" id="idm140658633245664" className="calibre7"></a>bastante fácil en D3, y comparten algunas similitudes con CSS en que calcular un nuevo retraso para cada elemento. Si está familiarizado con <code>for</code> bucles en JavaScript, esta implementación es probable que resultará familiar:</p>

<p className="p">Staggers are <a contenteditable="false" data-primary="staggers" data-type="indexterm" id="idm140658633245664" className="calibre7"></a>pretty easy in D3, and share some similarities with CSS in that you calculate a new delay for each element. If you’re familiar with <code>for</code> loops in JavaScript, this implementation will likely look familiar to you:</p>
