...
<p className="it">Vamos a aplicar esta animación a este muy simple <code className="calibre15">.ball</code>div (se muestra en la <a data-type="xref" href="part0007_split_001.html#fig2.1" className="calibre7">Figura 2-1</a> ):</p>

<p className="p">Let’s apply this animation to this very simple <code className="calibre15">.ball</code> div (shown in <a data-type="xref" href="part0007_split_001.html#fig2.1" className="calibre7">Figure 2-1</a>):</p>
...
<p className="it">Para obtener más información y más detalles acerca de cada propiedad de animación, como lo <code className="calibre15">animation-fill-mode</code>es, ¿qué facilidades están disponibles en CSS y cuyas propiedades son animatable, ver <a className="calibre7" href="http://shop.oreilly.com/product/0636920041658.do"><em className="calibre2">transiciones y animaciones en CSS</em></a> por Estelle Weyl (O'Reilly).</p>

<p className="p">For more information and further detail about each animation property, such as what <code className="calibre15">animation-fill-mode</code> is, what eases are available in CSS, and which properties are animatable, see <a className="calibre7" href="http://shop.oreilly.com/product/0636920041658.do"><em className="calibre2">Transitions and Animations in CSS</em></a> by Estelle Weyl (O’Reilly).</p>
...
<p className="it">Definimos el radio como medio de 50, por lo que 25 px. Entonces movemos el centro del círculo tanto en la x e y ejes ( <code className="calibre15">cx</code>y <code className="calibre15">cy</code>) a la mitad del radio, además de que el margen de 20 px hemos añadido en el CSS. También podríamos utilizar <code className="calibre15">margin</code>en el elemento SVG para moverlo, pero aquí estoy ilustrando que se puede dibujar coordenadas directamente en el propio SVG. Si movemos el círculo sobre, sin embargo, el&nbsp;<code className="calibre15">viewBox</code> <a contenteditable="false" data-primary="viewBox" data-secondary="sizing" data-type="indexterm" id="vb2s" className="calibre7"></a>tiene que ser más grande para acomodar estas coordenadas: es el ancho más el margen de espacio para el borde.</p>

<p className="p">We define the radius as half of 50, so 25 px. Then we move the center of the circle on both the x- and y-axes (<code className="calibre15">cx</code> and <code className="calibre15">cy</code>) to half the radius, plus that 20 px margin we added in the CSS. We could also use <code className="calibre15">margin</code> on the SVG element to move it, but here I’m illustrating that you can draw coordinates directly in the SVG itself. If we move the circle over, though, the <code className="calibre15">viewBox</code> <a contenteditable="false" data-primary="viewBox" data-secondary="sizing" data-type="indexterm" id="vb2s" className="calibre7"></a>has to be larger to accommodate these coordinates: it’s the width plus the margin of space to the edge.</p>

<p className="it">Ahora, si colocamos una clase sobre todo el llamado SVG <code className="calibre15">ball</code>, usando la misma declaración de animación, se obtiene lo que se muestra en <a data-type="xref" href="part0007_split_002.html#fig2.3" className="calibre7">la Figura 2-3</a> .</p>

<p className="p">Now, if we place a class on the whole SVG called <code className="calibre15">ball</code>, using the same animation declaration, we get what’s shown in <a data-type="xref" href="part0007_split_002.html#fig2.3" className="calibre7">Figure 2-3</a>.</p>
...
<p className="it">¿Lo que pasó aquí? Todavía se movió a través, ya que esperábamos. Pero el fondo se está llenando en el fondo por completo de la SVG, por lo tanto la totalidad <code className="calibre15">viewBox</code>. Eso no es realmente lo que queremos. Entonces, ¿qué ocurre si nos movemos esa clase y el objetivo del círculo en su lugar? Ver <a data-type="xref" href="part0007_split_002.html#fig2.4" className="calibre7">la Figura 2-4</a> .</p>

<p className="p">What happened here? It still moved across, as we were expecting. But the background is filling in the full background of the SVG, thus the entire <code className="calibre15">viewBox</code>. That’s not really what we want. So what happens if we move that class and target the circle instead? See <a data-type="xref" href="part0007_split_002.html#fig2.4" className="calibre7">Figure 2-4</a>.</p>
...
<ol className="calibre41">
  ...
  <p className="it">El <code className="calibre15">circle</code>se está moviendo dentro de la <code className="calibre15">viewBox</code>. Recuerde, si nos movemos un atributo SVG interna, la <code className="calibre15">viewBox</code>voluntad, literalmente, ser una ventana a través de la cual vemos estos elementos. Así que si movemos el <code className="calibre15">circle</code>sin que el <code className="calibre15">viewBox</code>lo suficientemente grande como para dar cabida a esas coordenadas, se cortó cuando el círculo se mueve fuera de la<a contenteditable="false" data-primary="viewBox" data-secondary="sizing" data-startref="vb2s" data-type="indexterm" id="idm140658633935040" className="calibre7"></a><code className="calibre15">viewBox</code>.</p>

  <p className="p">The <code className="calibre15">circle</code> is moving inside the <code className="calibre15">viewBox</code>. Remember, if we move an internal SVG attribute, the <code className="calibre15">viewBox</code> will quite literally be a window through which we view these elements. So if we move the <code className="calibre15">circle</code> without making the <code className="calibre15">viewBox</code> large enough to accommodate those coordinates, it will be cut off when the circle moves out of the <a contenteditable="false" data-primary="viewBox" data-secondary="sizing" data-startref="vb2s" data-type="indexterm" id="idm140658633935040" className="calibre7"></a><code className="calibre15">viewBox</code>.</p>
  </li>
  <li value="2" className="calibre10">
  <p className="it">El DOM SVG parece que el DOM HTML, pero es un poco diferente. No utilizamos <code className="calibre15">background</code>en los atributos SVG; utilizamos <code className="calibre15">fill</code>y <code className="calibre15">stroke</code>. Una hoja de estilo externa también tendrá un tiempo difícil anulando lo que se define dentro de la línea SVG. Así que vamos a echar un vistazo a la <code className="calibre15">fill</code>definición,<a contenteditable="false" data-primary="fill" data-type="indexterm" id="fill2" className="calibre7"></a>y mover eso en nuestra hoja de estilo.</p>

  <p className="p">The SVG DOM looks like the HTML DOM, but it’s slightly different. We don’t use <code className="calibre15">background</code> on SVG attributes; we use <code className="calibre15">fill</code> and <code className="calibre15">stroke</code>. An external stylesheet will also have a hard time overriding what is defined inline within the SVG. So let’s take out the <code className="calibre15">fill</code> definition, <a contenteditable="false" data-primary="fill" data-type="indexterm" id="fill2" className="calibre7"></a>and move that into our stylesheet.</p>
  </li>
</ol>
