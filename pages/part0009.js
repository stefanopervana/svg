...
<p className="it">Todos los elementos se asignan nombres de ID semánticas como “montaña” o “puente”. Las formas más detalladas también obtener una identificación compartida que podemos mostrar progresivamente para ventanas más grandes. Si la primera ilustración es <code className="calibre15">kells1</code>, el grupo particular a la segunda ilustración es <code className="calibre15">kells2</code>, y el último es <code className="calibre15">kells3</code>.</p>


<p className="p">All of the elements are assigned semantic ID names such as “mountain” or “bridge.” The most detailed shapes also get a shared ID that we can progressively show for larger viewports. If the first illustration is <code className="calibre15">kells1</code>, the group particular to the second illustration is <code className="calibre15">kells2</code>, and the last is <code className="calibre15">kells3</code>.</p>
...
<p className="it">Hay una captura, incluso si asignamos la capa inferior de una clase y ocultarlo, habrá un espacio vacío en el que <code className="calibre15">viewBox</code>sigue representando ese espacio. Con el fin de dar cuenta de esa zona, podemos cambiar el<a contenteditable="false" data-primary="viewBox" data-secondary="adjusting" data-type="indexterm" id="v4a" className="calibre7"></a> <code className="calibre15">viewBox</code> en el SVG para mostrar sólo la parte superior:</p>


<p className="p">There is one catch—even if we assign the bottom layer a class and hide it, there will be an empty gap where the <code className="calibre15">viewBox</code> still accounts for that space. In order to account for that area, we can change the<a contenteditable="false" data-primary="viewBox" data-secondary="adjusting" data-type="indexterm" id="v4a" className="calibre7"></a> <code className="calibre15">viewBox</code> in the SVG to show only the top portion:</p>

<pre className="programlisting">{`
viewBox="0 0 490 474"
`}</pre>

<p className="it">Eso funcionará, pero sólo para las dos versiones más grandes. La versión más pequeña está oscurecida, como <code className="calibre15">viewBox</code>está proporcionando una ventana a otra parte de la hoja de sprites SVG, por lo que tendrá que ajustarlo. Esto es similar a cambiar la posición de fondo en CSS para mostrar diferentes áreas de una hoja de sprites. Pero debido a que estamos ajustando un atributo SVG, necesitaremos JavaScript, como CSS todavía no tiene esa capacidad:</p>


<p className="p">That will work, but only for the two larger versions. The smallest version is now obscured, as the <code className="calibre15">viewBox</code> is providing a window into another portion of the SVG sprite sheet, so we will need to adjust it. This is akin to changing the background position in CSS to show different areas of a sprite sheet. But because we’re adjusting an SVG attribute, we will need JavaScript, as CSS doesn’t yet have that capability:</p>
